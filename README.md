...
- [Date Constructors](07_Constructors_Other_Tools/01_Constructors/07_0_1_Date_Constructors.cpp) - Shows constructor usage to initialize objects and explicit constructor calls 
- [Pitfall: No Default Constructor](07_Constructors_Other_Tools/01_Constructors/07_0_2_Pitfall_No_Default_Constructor.cpp) - Defining parameterized constructors prevents automatic default constructor generation.
- [Bank Account](07_Constructors_Other_Tools/01_Constructors/07_0_3_Bank_Account.cpp) - `BankAccount` class with overloaded constructors handling balance and interest rate.
- [Holiday](07_Constructors_Other_Tools/01_Constructors/07_0_4_Holiday.cpp) - Using class-type member variables initialized via constructor.
- [Member Initializers](07_Constructors_Other_Tools/01_Constructors/07_0_5_Member_Initializers.cpp) - Demonstrates C++11 member initialization syntax to set default values for member variables 
- [Constructor Delegation](07_Constructors_Other_Tools/01_Constructors/07_0_6_Construcor_Delegation.cpp) - Constructor delegation where one constructor calls another constructor to avoid code duplication.
...
- [Overloading Increment Decrement](08_Operator_Overloading_Friends_References/03_References_Overloaded_operators/08_0_6_Overloading_Increment_Decrement.cpp) - Showcases overloading of increment and decrement operators in both prefix (`++a`) and postfix (`a++`) notation for an `IntPair` class; prefix returns by reference to allow chaining, postfix returns by value.
- [Overloading Array Operator](08_Operator_Overloading_Friends_References/03_References_Overloaded_operators/08_0_7_Overloading_Array_operator.cpp) - Demonstrates overloading the array subscript operator `[]` for a `CharPair` class to enable indexed access to private members with bounds checking and error handling.
...
- [Dynamic Variables, Pointers](10_Pointers_Dynamic_arrays/01_Pointers/10_0_1_Dynamically_allocated_variable_Pointers.cpp) - Demonstrates pointer manipulation and dynamic variable allocation using `new`.
- [Pitfall: Call-by-value pointers](10_Pointers_Dynamic_arrays/01_Pointers/10_0_2_Pitfall_Call_by_value_pointers.cpp) - Demonstrates how passing a pointer by value to a function allows modification of the value pointed to, but not the pointer itself, and highlights the potential for unexpected side effects.
- [Avoid Dangling Pointers](10_Pointers_Dynamic_arrays/01_Pointers/10_99_Avoid_dangling_pointers.cpp) - Shows a strategy to avoid dereferencing dangling pointers by checking for `nullptr`.
1. [Pointers](10_Pointers_Dynamic_arrays/01_Pointers/10_1_Pointers.cpp) - Explains what a pointer is in C++ and its basic usage.
2. [Pointers Handling](10_Pointers_Dynamic_arrays/01_Pointers/10_2_Pointers_Use.cpp) - Shows at least three uses of the `*` operator with pointers.
3. [Pointers and Dynamic variables](10_Pointers_Dynamic_arrays/01_Pointers/10_3_Pointers_Dynamic_variables.cpp) - Explores pointer assignment, dynamic allocation, and output changes based on pointer manipulation.
4. [Pointers and Dynamic variables 2](10_Pointers_Dynamic_arrays/01_Pointers/10_4_Pointers_Dynamic_variables2.cpp) - Demonstrates pointer dereferencing and assignment, highlighting differences in output.
5. [Pitfall Wrong type ](10_Pointers_Dynamic_arrays/01_Pointers/10_05_Pitfall_Wrong_type.cpp) - Explains a common pitfall in pointer declarations where only the variable with `*` is a pointer.
6. [Destroy Dynamic variables](10_Pointers_Dynamic_arrays/01_Pointers/10_06_Destroy_Dynamic_variable.cpp) - Shows how to destroy a dynamically allocated variable and free its memory.
7. [Define pointer types with Typedef](10_Pointers_Dynamic_arrays/01_Pointers/10_07_Typedef.cpp) - Demonstrates using `typedef` to define a pointer type for dynamic variables.
8. [New operator](10_Pointers_Dynamic_arrays/01_Pointers/10_08_New_operator.cpp) - Describes the action of the `new` operator and how errors are indicated.
...
- [Multidimensional Dynamic Arrays](10_Pointers_Dynamic_arrays/02_Dynamic_Arrays/10_0_7_Multidimensional_dynamic_arrays.cpp) - Demonstrates how to create, fill, display, and properly delete a two-dimensional dynamic array (array of arrays) using pointers and dynamic memory allocation in C++.
...
- [Digital Time representation change](11_Separate_Compilation_Namespaces/01_Separate_compilation/11_5_Digital_Time_Change.cpp) - Describes, in words, how to modify the original `DigitalTime` interface and implementation to use a single `minute` field internally and summarizes the concrete changes realized in `11_5_dTime.h` / `11_5_dTime.cpp`.
...
- [Employee hierarchy](14_Inheritance/01_Inheritance_basics/14_0_01_Employee/14_0_01_Application.cpp) - Defines a base class `Employee` and derived classes `HourlyEmployee` and `SalariedEmployee`; constructs example objects and prints formatted paychecks to illustrate basic public inheritance and function overriding.
1. [Show employee data](14_Inheritance/01_Inheritance_basics/14_01_Employee_Show_data/14_01_Employee_Show_data.cpp) - Uses a function taking `const Employee&` to show that objects of derived classes (`HourlyEmployee`, `SalariedEmployee`) can be passed where a base-class reference is expected, printing common data for any `Employee`.
2. [SmartBut definition](14_Inheritance/01_Inheritance_basics/14_02_Smart_But/14_02_Smart_But.h) - Declares `SmartBut` as a derived class of `Smart` that adds a `bool crazy` data member, an `isCrazy()` accessor, and suitable constructors, illustrating how to extend a base class.
3. [Is Crazy implementation](14_Inheritance/01_Inheritance_basics/14_03_Is_Crazy/14_03_Application.cpp) - Implements `Smart`/`SmartBut` and tests `isCrazy()`, demonstrating that a derived class can access `protected` data members of its base class and discussing legality vs design quality of such access.
4. [Redefining functions](14_Inheritance/01_Inheritance_basics/14_04_Redefining_functions/14_04_Redefining_functions.cpp) - Explains why a derived class like `SalariedEmployee` only needs to redeclare and redefine functions (such as `printCheck`) it changes, while inherited functions like `getName` are reused without redeclaration.
5. [Titled employee interface](14_Inheritance/01_Inheritance_basics/14_05_Additional_class/14_05_Titled_Employee.h) - Designs a `TitleEmployee` class derived from `SalariedEmployee` that adds a `title` string, `getTitle`/`setTitle` functions, and a redefined `setName` that incorporates the title.
6. [Titled employee implementation](14_Inheritance/01_Inheritance_basics/14_06_Titled_Employee/14_06_Application.cpp) - Implements `TitleEmployee` constructors and `setName` using the base `Employee::setName`, then constructs an example object and prints its paycheck to show how derived classes can extend and reuse base behavior.
7. [Rule of three in inheritance](14_Inheritance/01_Inheritance_basics/14_07_Rule_Three_Inheritance/14_07_Rule_Three_Inheritance.cpp) - Discusses copy constructors and assignment operators in derived classes, noting that if you do not define them, the compiler still generates default versions (often inadequate for classes managing dynamic resources).
